...
if keyboard_check(ord("D")) and !instance_place(x + move_speed, y, obj_Barrier) and !self.disable_east and x + move_speed < room_width - 90 {
	x += self.move_speed * abs(image_xscale); // Funny exponential get smaller when falling
	image_xscale = abs(image_xscale);
}
...
if keyboard_check(ord("S")) and !instance_place(x, y + move_speed, obj_Barrier) and !self.disable_south and y + move_speed < room_height - 90 {
	y += self.move_speed * image_yscale; // Funny exponential get smaller when falling
}
